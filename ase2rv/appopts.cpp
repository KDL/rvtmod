...
void AppOptions::parse( int argc, char **argv )
{
  ...
  for(i=1; i<argc; i++)
  {
    if(argv[i][0]=='-')
    {
      if(!_strcmpi(argv[i], "-l"))
      {
        listobj_only=true;
      }
      else if(!_strcmpi(argv[i], "-v"))
      {
        msglevel=2;
      }
      else if(!_strcmpi(argv[i], "-q"))
      {
        msglevel=0;
      }
      else if(!_strcmpi(argv[i], "-ase"))
      {
        aprintf=asemode_printf;
      }
      else if(!_strcmpi(argv[i], "-o="))
      {
        if(out_name[0])
      ...
      else if((cmd==CmdWorld || cmd==CmdPrm) && (!_strcmpi(argv[i], "-morph") || !_strcmpi(argv[i], "-m")))
      {
        if(wmode!=WmNone)
          cmderr("-ali|-morph specified multiple times");
        wmode=WmMorph;
      }
      else if((cmd==CmdWorld || cmd==CmdPrm) && (!_strcmpi(argv[i], "-ali") || !_strcmpi(argv[i], "-a")))
      {
        if(wmode!=WmNone)
          cmderr("-ali|-morph specified multiple times");
        wmode=WmAli;
      }
      else if((cmd==CmdWorld || cmd==CmdPrm) && !_strcmpi(argv[i], "-sg"))
      {
        sgdump_only=true;
        return;
      }
      else if((cmd==CmdPrm) && !_strcmpi(argv[i], "-c"))
      {
        center_grip=true;
      }
      else if((cmd==CmdVis || cmd==CmdTaz))
      {
        if(safety_dist)
      ...
      else if((cmd==CmdVis || cmd==CmdTaz) && !_strcmpi(argv[i], "-sy"))
      {
...
void AppOptions::welcome()
{
  if(cmd==CmdWorld || cmd==CmdPrm)
  {
    ...
    lprintf(1, " ------------------------------------------------------------------------------\n");
	lprintf(1, "\______________________ Last update: August'6th 2012 by Kallel A.Y______________/");
  }
  else
  {
    /* 80:      123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 */
